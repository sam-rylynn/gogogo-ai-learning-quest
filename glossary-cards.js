(() => {
  ...
  const TERMS = {
    evals: {
      ...
      aliases: ["Evals", "Eval"],
      summary:
        "先写好检查表，再用几份不同材料试一试，看 AI 有没有做对，而不只看回答是否流畅。",
      logic:
      ...
      example:
        "让 AI 写活动通知，用检查表核对时间、地点和字数。再换一份地点未定的材料，检查它会不会擅自补造地点。"
    },
    prompt: {
      term: "Prompt",
      ...
      summary:
        "你发给 AI 的要求和材料。说清要做什么、给谁看、哪些信息已知，比寻找一句万能口令更有用。",
      logic:
      ...
      example:
        "“帮我写通知”太模糊。可以说：“给读书会成员写活动通知，周六下午两点，地点待定，100 字以内。没有提供的信息不要编。”"
    },
    llm: {
      term: "LLM",
      ...
      summary:
        "从大量文字中学习规律、能理解和生成语言的模型。它能起草、总结和解释，但说得流畅不等于事实正确。",
      logic:
      ...
      example:
        "让 AI 把读书会记录整理成待办，它可以帮忙分组和表达，但不能猜出记录里没有写的负责人。"
    },
    rag: {
      term: "RAG",
      ...
      example:
        "问图书馆能借几本书时，先找到现行借阅说明，再据此回答并附上出处；资料没有写的内容不能自行补齐。"
    },
    toolCalling: {
      term: "Tool Calling",
      ...
      example:
        "你请助手查周六的日历。模型提出查询日期，日历工具返回安排，助手再整理结果。说“查过了”不等于真的执行了查询。"
    },
    memory: {
      term: "Memory",
    ...
    ruleEngine: {
      ...
      example:
        "活动报名截止后不再接受新报名，可以由固定规则判断日期，不让 AI 随意决定是否放行。"
    },
    hitl: {
      term: "HITL",
      ...
      example:
        "AI 写好群通知后，由组织者核对时间、地点和收件人，再确认发送，而不是生成完就自动群发。"
    },
    api: {
      term: "API",
      ...
      example:
        "天气应用通过天气服务的接口，按城市获取预报。接口是取资料的通道，预报由天气服务提供。"
    },
    json: {
      term: "JSON",
      ...
      example:
        "一本书的资料可以写成 {\"title\":\"小王子\",\"available\":true}，其中 title 是标题，available 表示是否可借。"
    },
    http: {
      term: "HTTP",
      ...
      example:
        "打开网页就是一次请求。200 通常表示请求成功，404 表示没有找到目标资源，500 表示服务器出了问题；状态码不能单独证明返回内容正确。"
    },
    git: {
      term: "Git",
    ...
    transformer: {
      ...
      example:
        "在“小林把书借给小李，他明天归还”这句话中，模型需要联系前后文字判断“他”指谁；若仍有歧义，应向你确认。"
    },
    token: {
      term: "Token",
      translation: "词元 / 模型处理文本的基本片段",
    ...
    hallucination: {
      ...
      example:
        "材料只写“地点待定”，AI 却把通知写成“在市图书馆见”，就是把没有依据的信息补成了事实。"
    },
    fineTuning: {
      term: "Fine-tuning",
      translation: "微调",
      ...
      example:
        "可以用一批好例子训练模型按固定栏目写周报，但本周新发生的事仍要提供材料，不能靠训练让它自动知道。"
    },
    agent: {
      term: "AI Agent",
      ...
      logic:
        "它会根据目标选择下一步，借助可用工具做事，再查看结果。能做什么取决于工具与权限，重要操作仍需要人确认。",
      flow: ["接收目标", "分析状态并规划", "选择工具执行", "读取并判断结果", "必要时调整计划", "完成、停止或转人工"],
      where: ["研究与资料整理", "客服流程", "代码任务", "运营自动化", "多步骤业务处理"],
      example:
        "你请助手整理读书会资料，它先找文件、提取待办，再根据检查结果补充缺项。是否真的改好文件，要看实际结果，不能只听它说完成。"
    }
  };
  ...
  function ensureCard() {
    ...
    card.innerHTML = `
      <section class="gc-card" role="dialog" aria-modal="true" aria-labelledby="gc-title">
        ...
        <details class="gc-more"><summary>继续了解原理与使用场景</summary><section class="gc-section">
          <h3>工作逻辑</h3>
          <p id="gc-logic"></p>
        </section>
  ...
  function openCard(key, trigger) {
    ...
    overlay.querySelector("#gc-title").textContent = data.translation.split(/[；/]/)[0].trim();
    overlay.querySelector('.gc-more').open = false;
    overlay.querySelector("#gc-translation").textContent = data.term + " · " + data.translation;
